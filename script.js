...
// ===== FUNGSI BELAJAR (INDEX) =====
function renderKelasGrid() {
    const grid = document.getElementById('kelas-grid');
    if (!grid) return;
    grid.innerHTML = kelasData.map(k => `
        <div class="kelas-card" onclick="selectKelas('${k.nama}')">
...
function selectKelas(kelasNama) {
    ...
    const mapelSection = document.getElementById('mapel-section');
    const materiSection = document.getElementById('materi-section');

    if (kelasSection) kelasSection.style.display = 'none';
    if (mapelSection) mapelSection.style.display = 'block';
    if (materiSection) materiSection.style.display = 'none';

    const title = document.getElementById('selected-kelas-title');
    if (title) title.textContent = kelasNama;

    const mapels = mapelData[kelasNama] || [];
    const mapelGrid = document.getElementById('mapel-grid');
    if (mapelGrid) {
        mapelGrid.innerHTML = mapels.map(m => `
            <div class="mapel-card" onclick="selectMapel('${m.nama}')">
                <div class="mapel-icon"><i class="fas ${m.icon}"></i></div>
                <h4>${m.nama}</h4>
                <p>${m.deskripsi}</p>
            </div>
        `).join('');
    ...
    if (mapelSection) mapelSection.scrollIntoView({ behavior: 'smooth' });
}
...
function backToKelas() {
    ...
    if (mapelSection) mapelSection.style.display = 'none';
    if (materiSection) materiSection.style.display = 'none';
}
...
function selectMapel(mapelNama) {
    ...
    if (mapelSection) mapelSection.style.display = 'none';
    if (materiSection) materiSection.style.display = 'block';

    const kelasBadge = document.getElementById('materi-kelas-badge');
    const mapelBadge = document.getElementById('materi-mapel-badge');
    if (kelasBadge) kelasBadge.textContent = selectedKelas;
    if (mapelBadge) mapelBadge.textContent = selectedMapel;
    ...
    if (materiContent) {
        materiContent.innerHTML = `
            <div class="empty-state">
                ...
                <h4>Siap Belajar?</h4>
                <p>Masukkan topik yang ingin kamu pelajari, lalu klik "Generate Materi" untuk mendapatkan penjelasan lengkap dari AI.</p>
            </div>
        `;
    }

    const topikInput = document.getElementById('topik-input');
...
async function generateMateri() {
    ...
    if (!topik) { showToast('Masukkan topik yang ingin dipelajari'); return; }
    if (!selectedKelas || !selectedMapel) { showToast('Pilih kelas dan mapel dulu'); return; }
    ...
    contentDiv.innerHTML = `
        <div class="loading-state">
            ...
            <p>AI sedang menyusun materi...</p>
            <small>Tunggu sebentar ya</small>
        </div>
    `;
...
document.addEventListener('DOMContentLoaded', () => {
    initNavbar();
    if (document.getElementById('kelas-grid')) {
        renderKelasGrid();
    }
});
